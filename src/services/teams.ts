...
export const teamsService = {
  ...
  create: (dto: CreateTeamDto) =>
    http<Team>('/teams', { method: 'POST', body: JSON.stringify(dto) }),
  update: (id: string, dto: UpdateTeamDto) =>
    http<Team>(`/teams/${id}`, { method: 'PATCH', body: JSON.stringify(dto) }),
  delete: (id: string) => http<void>(`/teams/${id}`, { method: 'DELETE' }),
  addMember: (id: string, dto: AddTeamMemberDto) =>
    http<void>(`/teams/${id}/members`, {
      method: 'POST',
      body: JSON.stringify(dto),
    }),
  removeMember: (id: string, userId: number) =>
    http<void>(`/teams/${id}/members/${userId}`, { method: 'DELETE' }),
};
